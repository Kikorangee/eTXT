...
function md5hex(str) {
  ...
  function core(q, a, b, x, shift, t) {
    return safeAdd(rotL(safeAdd(safeAdd(a, q), safeAdd(x, t)), shift), b);
  }
...
async function buildHmacAuth(method, pathAndQuery, bodyText) {
  const date = new Date().toUTCString();
  let signingString, authHeaderValue, contentMd5Header;

  if (bodyText) {
    contentMd5Header = hexToBase64(md5hex(bodyText));
...
export default {
  async fetch(request) {
    ...
    const { date, contentMd5Header, authHeaderValue } = await buildHmacAuth(
      request.method, pathAndQuery, bodyText
    );
